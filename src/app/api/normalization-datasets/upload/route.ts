...
export async function POST(request: Request) {
  try {
    console.log('=== Normalization Dataset Upload API Called ===');
    ...
    // Create R2 key in normalization folder structure: normalization/{userId}/{filename}
    const r2Key = `normalization/${session.user.id}/${fileName}`;
    ...
    const metadata = {
      id: `norm_${transformedName}_${timestamp}`,
      ...
      datasetType: 'normalization',
      uploadedBy: session.user.id
    };

    // Upload metadata JSON to R2
    const metadataKey = `metadata/normalization/${session.user.id}/${fileName.replace('.csv', '.json')}`;
    ...
    const metadataUploadCommand = new PutObjectCommand({
      ...
      Metadata: {
        'uploaded-by': session.user.id,
        'upload-timestamp': timestamp.toString(),
        'metadata-type': 'normalization-dataset'
      },
    });
    ...
    console.log('=== Normalization Dataset Upload Successful ===');

    return NextResponse.json({
      success: true,
      message: 'Dataset saved to normalization folder successfully',
      dataset: metadata
    });

  } catch (error) {
    console.error('Error uploading normalization dataset:', error);
    return NextResponse.json({ 
