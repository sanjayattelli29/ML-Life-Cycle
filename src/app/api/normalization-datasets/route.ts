...
export async function GET() {
  try {
    console.log('=== Normalization Datasets API Called ===');
    ...
    // List all CSV files for this user in the normalization folder
    const listCommand = new ListObjectsV2Command({
      Bucket: process.env.R2_BUCKET_NAME!,
      Prefix: `normalization/${session.user.id}/`,
    });
    ...
    if (response.Contents) {
      console.log(`Found ${response.Contents.length} files in normalization folder for user`);
      ...
      for (const object of response.Contents) {
        if (object.Key && object.Key.endsWith('.csv')) {
          ...
          datasets.push({
            id: `norm_${transformedName}_${Date.now()}`,
    ...
    console.log(`Total normalization datasets found: ${datasets.length}`);
    ...
    console.error('Error fetching normalization datasets:', error);
    return NextResponse.json({ 
