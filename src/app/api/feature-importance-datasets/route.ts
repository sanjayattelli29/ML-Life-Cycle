...
export async function GET() {
  try {
    console.log('=== Feature-Importance Datasets Fetch API Called ===');
    ...
    console.log('Fetching feature-importance datasets for user:', session.user.id);

    // List objects in feature-importance/{userId}/ folder
    const listCommand = new ListObjectsV2Command({
      Bucket: process.env.R2_BUCKET_NAME!,
      Prefix: `feature-importance/${session.user.id}/`,
      Delimiter: '/'
    });
    ...
    if (!result.Contents || result.Contents.length === 0) {
      console.log('No feature-importance datasets found for user');
      return NextResponse.json({ 
        datasets: [],
        message: 'No feature-importance datasets found'
      });
    }
    ...
    for (const file of csvFiles) {
      ...
      try {
        ...
        const metadataKey = file.Key
          .replace('feature-importance/', 'metadata/feature-importance/')
          .replace('.csv', '.json');
        ...
        const basicDataset = {
          id: `feat_${nameWithoutExt}_${file.LastModified?.getTime() || Date.now()}`,
          ...
          rowCount: null,
          columnCount: null,
          processingSteps: ['feature_importance'],
    ...
    console.log('Returning feature-importance datasets:', datasets.length);
    console.log('=== Feature-Importance Datasets Fetch Complete ===');

    return NextResponse.json({
      datasets,
      count: datasets.length,
      message: `Found ${datasets.length} feature-importance dataset(s)`
    });

  } catch (error) {
    console.error('Error fetching feature-importance datasets:', error);
    return NextResponse.json({ 
      error: 'Failed to fetch feature-importance datasets',
