...
export async function POST(request: Request) {
  try {
    console.log('=== Model-Training Dataset Upload API Called ===');
    ...
    // Create R2 key in model-training folder structure: model-training/{userId}/{filename}
    const r2Key = `model-training/${session.user.id}/${fileName}`;
    ...
    const datasetMetadata = {
      id: `model_${transformedName}_${timestamp}`,
    ...
    const metadataKey = `metadata/model-training/${session.user.id}/${fileName.replace('.csv', '.json')}`;
    console.log('Uploading metadata to:', metadataKey);

    const metadataUploadCommand = new PutObjectCommand({
      Bucket: process.env.R2_BUCKET_NAME!,
      Key: metadataKey,
      Body: Buffer.from(JSON.stringify(datasetMetadata, null, 2)),
      ContentType: 'application/json',
      Metadata: {
        'uploaded-by': session.user.id,
        'upload-timestamp': timestamp.toString(),
        'metadata-type': 'model-training-dataset'
      },
    });
    ...
    console.log('=== Model-Training Dataset Upload Successful ===');

    return NextResponse.json({
      success: true,
      message: 'Dataset saved to model-training folder successfully',
      dataset: datasetMetadata
    });

  } catch (error) {
    console.error('Error uploading model-training dataset:', error);
    return NextResponse.json({ 
